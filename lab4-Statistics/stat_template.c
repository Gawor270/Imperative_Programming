...
void bernoulli_gen(int v[], int n, double probability) {
	for(int i=0; i<n; ++i){
		if(rand_from_interval(0,RAND_MAX) < (probability*RAND_MAX + probability)) v[i]=1;
		else v[i]=0;
	}
}
...
void pmf(double v[], int trials) {
	...
	for(int i=0; i<trials; ++i){
		int dice1 = rand_from_interval(1,6);
		int dice2 = rand_from_interval(1,6);
		v[dice1+dice2-2]++;
	}
	for(int i=0; i<TWO_DICE_SUM; ++i){
		v[i]/=trials;
...
void print_histogram(const double v[], int n, int x_start, double y_scale, char mark) {
	for(int i=x_start; i<x_start+n; ++i){
		...
		for(int j=0; j<round(v[i-2]/y_scale);j++){
			printf("%c",mark);
		}
		printf(" %.3f\n", v[i-2]);
	}
...
void monty_hall(int n, int *p_switch_wins) {
	int counter=0;
	for(int i=0; i<n; i++){
		int prize = rand_from_interval(1,3);
		int player = rand_from_interval(1,3);
		if(player!=prize) counter++;
	}
	*p_switch_wins = counter;
}
